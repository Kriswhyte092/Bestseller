...
class Command(BaseCommand):
    ...
    def on_exit(self):
        ...
        self.stdout.write(self.style.ERROR(f"Processing exited after {elapsed_time:.2f} seconds or {elapsed_time/60:.2f} minutes"))
    ...
    def process_variant_stock(self, data):
        ...
        with transaction.atomic():
            for record in tqdm(data["value"], desc="Processing records", unit="variant", ascii="_#", colour="blue"):
                try:
                    ...
                    if not product:
                        ...
                        logging.info(f"New Product: {product}")
                        if not product:
                    ...
                    store.save()
                    logging.info(f"Store: {store}, New: {created}")
                    ...
                    if not variant:
                        ...
                        self.stdout.write(self.style.ERROR(f"Variant {variant_code} for Product {item_no} does not exist."))
                        continue
                    ...
                    logging.info(f"Inventory: {inventory}, Created: {created}")
                    if not created:
                        logging.info(f"Inventory exists and updated - {store} / {variant.colorVariant.colorName} {inventory}")
        ...
        elapsed_time = time.time() - self.start_time
        logging.info(f"Processing completed in {elapsed_time:.2f} seconds or {elapsed_time/60:.2f} minutes - {len(data)} records processed")
