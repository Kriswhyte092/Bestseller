            ...
            for key, value in current.items():
                ...
                if key == 'options' and isinstance(value, list):
                    for color_info in value:
                        ...
                        product.color_variants.append(color_variant)
...
for code in JJ_noos_barcodes:
    ...
    if fashion_cloud_product is None:
        print(f"Error fetching data for barcode: {code}")
        continue
    p, v = import_product(fashion_cloud_product)
    if p:
        product_count += 1
    if v:
        color_variant_count += len(p.color_variants)
        variant_count += sum(len(cv.variants) for cv in p.color_variants)
    console.print(p, style="purple")

end_time = time.time()
console.print(f"Elapsed time: {end_time - start_time} seconds")
console.print(f"Total products: {product_count}")
console.print(f"Total color variants: {color_variant_count}")
console.print(f"Total variants: {variant_count}")